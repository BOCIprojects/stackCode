...
<style type="text/css">
	...
	var initialLoad = true; // Nothing
		...
		var dataSource = new kendo.data.DataSource({
			transport: {
				read: {
					url: "/ermsweb/getDataByccdId?ccdId="+paramCCDID,//"/ermsweb/resources/js/getClientCounterDetails.json",
					dataType: "json"
				},
				update:{
					type:"POST",
					url: "/ermsweb/updateDataByccdId",//"/ermsweb/updateDataByccdId",
					dataType: "json",
					contentType:"application/json",
					complete: function (jqXHR, textStatus){
						var response = jqXHR.responseText;
						window.location.href= "/ermsweb/"+response;
					}
				},
			...
			var categoryData = [
      		...
      		$("#saveBtn").kendoButton({
    			click: function(){
    				...
    				var updateCriteria = {
   						clnCptyCat:$("#cc_category").val(),creditRatingInternal:$("#credit_rating_internal_private").val(),externalKey1:$("#checkEXTKey1 input").val(),externalKey2:$("#checkEXTKey2 input").val(),externalKey3:$("#checkEXTKey3 input").val(),externalKey4:$("#checkEXTKey4 input").val(),externalKey5:$("#checkEXTKey5 input").val(),externalKey6:$("#checkEXTKey6 input").val(),externalKey7:$("#checkEXTKey7 input").val(),externalKey8:$("#checkEXTKey8 input").val(),externalKey9:$("#checkEXTKey9 input").val(),externalKey10:$("#checkEXTKey10 input").val()
   					}
    				
    				var dataDatasource = dataSource.at(0);
    				dataDatasource.set("clnCptyCat",updateCriteria.clnCptyCat);
    				dataDatasource.set("creditRatingInternal",updateCriteria.creditRatingInternal);
    				...
    				dataDatasource.set("status","save");
    				dataSource.sync();
    			}
    		});
      		...
      		$("#submitBtn").kendoButton({
    			click: function(){
    				...
    				dataDatasource.set("externalKey9",updateCriteria.externalKey9);
    				dataDatasource.set("externalKey10",updateCriteria.externalKey10);
    				dataDatasource.set("status","submit");
    				dataSource.sync();
    			}
    		});
		});	
		...
		var collateraldataSource = new kendo.data.DataSource({
			transport: {
				read: {
					url: "/ermsweb/getThirdPartyCollateraList?ccdId="+paramCCDID,//"/ermsweb/resources/js/getThirdPartlyCollateralList.json",
					dataType: "json"
				}
			},
			pageSize:1									
